...
var selectListShow = document.querySelectorAll('.display-column__list');
...
userInput.forEach(function(user){
    user.addEventListener('input',function(){
        ...
        this.parentNode.querySelector('.display-column__list').classList.add('display-column__list_visibility');
        var users= document.querySelectorAll('.display-column__list')[0].children.length;
//работа с массивом участников
var usersArray = [];
var search = event.target.value;
search = search.toLowerCase();
var users = this.parentNode.querySelectorAll('.display-column__list')[0].children;
for (var i=0;i<users.length; i++)
{   
    var li = document.querySelectorAll('.display-column__list')[0].children[i].innerText.toLowerCase();
  
        if (~(li.indexOf(search)) && (search.length!=0)) {
            this.parentNode.querySelectorAll('.display-column__list')[0].children[i].classList.add('display-column__li_fontWeigth');
        }
        else {
            this.parentNode.querySelectorAll('.display-column__list')[0].children[i].classList.remove('display-column__li_fontWeigth');
        }
        ...
        if (event.target.value.length==0){
            ...
            this.parentNode.querySelector('.display-column__list').classList.remove('display-column__list_visibility');
        }
